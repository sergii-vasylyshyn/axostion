...
const datepickers_init = () => {
    const elems = document.querySelectorAll('[data-datepicker]');
    let incrementID = 1;
    for (const elem of elems) {
        if ( !window.datepickers ) window.datepickers = {}
        if ( !elem.id ) elem.id = 'datepicker_'+incrementID;
        if ( !window.datepickers[elem.id] ) window.datepickers[elem.id] = {}
        if ( window.datepickers[elem.id]['init'] ) return false;

        window.datepickers[elem.id]['elem'] = elem;
        window.datepickers[elem.id]['datepicker'] = new Datepicker(elem, {
            format: 'dd-mm-yyyy',
            autohide: true,
            language: 'ru'
        });
        window.datepickers[elem.id]['init'] = true;
        incrementID++;
    }
}